...
# Refilter the data with less granularity (only see one measurement every hour)
houseMonth2 <- filter(df, year == 2008 &  (hour == 12 & minute == 0))
houseMonth2
```

```{r - Plot sub-meter 1, 2 and 3 with title, legend and labels - 10 Minute frequency}
plot_ly(houseMonth2, x = ~houseMonth2$DateTime, y = ~houseMonth2$Sub_metering_1, name = 'Kitchen', type = 'scatter', mode = 'lines') %>%
 add_trace(y = ~houseMonth2$Sub_metering_2, name = 'Laundry Room', mode = 'lines') %>%
 add_trace(y = ~houseMonth2$Sub_metering_3, name = 'Water Heater & AC', mode = 'lines') %>%
 layout(title = "Power Consumption January 9th, 2008",
 xaxis = list(title = "Time"),
 yaxis = list (title = "Power (watt-hours)"))
```
